...
def Layer_generation():
    ...
    if specList[0][10] == 1:
        inSize = specList[0][0] * specList[0][1] * specList[0][2] - 1
    else:
    ...
    for k in range(len(specList) + 1):
        ...
        if k == 0:
            if specList[k][10] == 1:
                memSize = specList[k][0] * specList[k][1] * specList[k][2] - 1
            else:
        ...
        # Other Regs
            ...
            if specList[k-1][11] == 1:
                actSize = specList[k-1][4] * specList[k-1][9] * specList[k-1][3] * specList[k-1][8] * specList[k-1][5] - 1
            else:
                actSize = memSize
    ...
    outfile.write("\talways @(posedge clk)\n")
    outfile.write("\tbegin\n")
    for k in range(1, len(specList)+1):
        outfile.write("\t\treg%d_mem_read_data <= reg%d_mem_write_data;\n" % (k, k))
    outfile.write("\t\treg0_mem_read_data <= reg0_mem_write_data;\n")
    outfile.write("\tend\n")
    outfile.write("\n")
