...
public:
	...
	vector<ap_uint<512> > return_vector();//used to avoid errors when passing data to hw side by opencl
private:
	...
	int pe;
	T ****matrix;
	vector<ap_uint<512> > data_vector;
	///////////////////////////////
	T ****soft_result;
	int result_kernel=1;
	int result_channel;
...
template<class T>
vector<ap_uint<512> > input_data<T>::return_vector(){
	return data_vector;
}
...
int main(int argc, char** argv){
	...
	input_data<ap_uint<WEIGHT_PRECISION> > W10(OFM_Channels_10,IFM_Channels_10,KERNELDim_10,WEIGHT,SIMD_10,PE_10);
	input_data<ap_uint<WEIGHT_PRECISION> > W11(OFM_Channels_11,IFM_Channels_11,KERNELDim_11,WEIGHT,SIMD_11,PE_11);
	input_data<ap_uint<WEIGHT_PRECISION> > W12(OFM_Channels_12,IFM_Channels_12,KERNELDim_12,WEIGHT,SIMD_12,PE_12);
	input_data<ap_uint<WEIGHT_PRECISION> > W13(OFM_Channels_13,IFM_Channels_13,KERNELDim_13,WEIGHT,SIMD_13,PE_13);
	input_data<ap_uint<WEIGHT_PRECISION> > W14(OFM_Channels_14,IFM_Channels_14,KERNELDim_14,WEIGHT,SIMD_14,PE_14);
	input_data<ap_uint<WEIGHT_PRECISION> > W15(OFM_Channels_15,IFM_Channels_15,KERNELDim_15,WEIGHT,SIMD_15,PE_15);
	input_data<ap_uint<WEIGHT_PRECISION> > W16(OFM_Channels_16,IFM_Channels_16,KERNELDim_16,WEIGHT,SIMD_16,PE_16);
	...
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM10;
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM11;
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM12;
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM13;
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM14;
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM15;
	input_data<ap_uint<ACTIVATION_PRECISION> > OFM16;
	...
	OFM10 = OFM9 * W10;
	OFM10.pooling();
	OFM10.matrix_padding();
	OFM11 = OFM10 * W11;
	OFM11.matrix_padding();
	OFM12 = OFM11 * W12;
	OFM12.matrix_padding();
	OFM13 = OFM12 * W13;
	OFM13.pooling();
	OFM14 = OFM13 * W14;
	OFM15 = OFM14 * W15;
    ...
    vector< ap_uint<WIDTH> > output_vector(out_size);
    vector< ap_uint<WIDTH> > IFM_(IFM.return_vector());
	vector< ap_uint<WIDTH> > W1_(W1.return_vector());
	vector< ap_uint<WIDTH> > W2_(W2.return_vector());
	vector< ap_uint<WIDTH> > W3_(W3.return_vector());
	vector< ap_uint<WIDTH> > W4_(W4.return_vector());
	vector< ap_uint<WIDTH> > W5_(W5.return_vector());
	vector< ap_uint<WIDTH> > W6_(W6.return_vector());
	vector< ap_uint<WIDTH> > W7_(W7.return_vector());
	vector< ap_uint<WIDTH> > W8_(W8.return_vector());
	vector< ap_uint<WIDTH> > W9_(W9.return_vector());
	vector< ap_uint<WIDTH> > W10_(W10.return_vector());
	vector< ap_uint<WIDTH> > W11_(W11.return_vector());
	vector< ap_uint<WIDTH> > W12_(W12.return_vector());
	vector< ap_uint<WIDTH> > W13_(W13.return_vector());
	vector< ap_uint<WIDTH> > W14_(W14.return_vector());
	vector< ap_uint<WIDTH> > W15_(W15.return_vector());
	vector< ap_uint<WIDTH> > W16_(W16.return_vector());
	...
	int weight10_size=W10_.size()*WIDTH/8;//byte
	int weight11_size=W11_.size()*WIDTH/8;//byte
	int weight12_size=W12_.size()*WIDTH/8;//byte
	int weight13_size=W13_.size()*WIDTH/8;//byte
	int weight14_size=W14_.size()*WIDTH/8;//byte
	int weight15_size=W15_.size()*WIDTH/8;//byte
	int weight16_size=W16_.size()*WIDTH/8;//byte
...
/*
	...
	OCL_CHECK(err, cl::Buffer buffer_weight10(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight10_size, W10_.data(), &err));
	OCL_CHECK(err, cl::Buffer buffer_weight11(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight11_size, W11_.data(), &err));
	OCL_CHECK(err, cl::Buffer buffer_weight12(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight12_size, W12_.data(), &err));
	OCL_CHECK(err, cl::Buffer buffer_weight13(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight13_size, W13_.data(), &err));
	OCL_CHECK(err, cl::Buffer buffer_weight14(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight14_size, W14_.data(), &err));
	OCL_CHECK(err, cl::Buffer buffer_weight15(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight15_size, W15_.data(), &err));
	OCL_CHECK(err, cl::Buffer buffer_weight16(context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, weight16_size, W16_.data(), &err));
	...
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight10));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight11));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight12));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight13));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight14));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight15));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_weight16));
	OCL_CHECK(err, err = one_layer.setArg(narg++, buffer_out));
//copy data from host to device
	cout<<"***host copy data from host to device***"<<endl;
	OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_IFM}, 0));
	OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_weight1,
													 ...
													 buffer_weight14,
													 buffer_weight15,
	//												 buffer_weight16}, 0  0 means from host//));
	// Launch the Kernel
